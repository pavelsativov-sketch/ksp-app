...
function stubKsp(input: GenerateKspInput): AiKspPayload {
  ...
  return {
    ...
    stages: {
      ...
      middle: {
        time: "11–35 мин",
        teacherActions: `Объяснение нового материала по теме «${topic}» с использованием наглядности. Организация работы в парах: ученики выполняют задание по образцу, затем — самостоятельно. Учитель задаёт наводящие вопросы, корректирует работу групп, раздаёт дифференцированные карточки. Физкультминутка в середине этапа.`,
        studentActions:
        ...
        time: "36–45 мин",
        teacherActions:
          "Подводит итог урока через приём «3 вещи, которые я сегодня узнал». Проводит рефлексию по технике «Лестница успеха». Даёт домашнее задание с комментарием. Благодарит за работу.",
        studentActions:
