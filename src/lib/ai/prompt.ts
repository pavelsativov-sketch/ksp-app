...
КЛЮЧЕВОЕ ТРЕБОВАНИЕ № 4 — ОБЩИЙ ДЕСКРИПТОР 1–10.
В поле overallRubric РОВНО 10 объектов { points, descriptor }, по одному на каждый балл от 1 до 10 в порядке возрастания. Каждый descriptor — это 1–2 коротких предложения о том, что именно умеет/демонстрирует ученик ПО ЭТОЙ КОНКРЕТНОЙ ТЕМЕ, набравший этот балл. Шкала равномерная: 1–2 балла — не приступил/не понимает; 3–4 — распознаёт отдельные элементы, но не связывает; 5–6 — выполняет базовые задания с поддержкой; 7–8 — самостоятельно решает типовые задачи; 9–10 — применяет изученное в новых контекстах, объясняет другим. Дескрипторы должны быть НАБЛЮДАЕМЫМИ и ПРОВЕРЯЕМЫМИ («Называет…», «Решает…», «Объясняет…»), не общие фразы вроде «Понимает тему».
...
- overallRubric (РОВНО 10 объектов { points, descriptor }, points = 1, 2, …, 10 в этом порядке): наблюдаемый дескриптор по теме урока для каждого балла от 1 до 10. Каждый descriptor начинается с глагола действия (Называет…, Объясняет…, Применяет…, Сравнивает…, Оценивает…, Переносит…).
